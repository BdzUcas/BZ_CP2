...
def main_triangle(startx,starty,size):
    ...
    del t
#triangle function
def triangle(startx,starty,size,depth,limit):
    ...
    triangle(startx + size/2, starty, size/2, depth + 1, limit)
    #draw top
    t.left(120)
    t.forward(size)
    #draw smaller triangle relative to top
    triangle(startx, t.ycor(), size/2, depth + 1, limit)
    #draw left side
    t.left(120)
    t.forward(size)
    ...
    del t
#koch line function
def koch_line(startx,starty,angle,dir,size,depth,limit):
    #create turtle object
    t = turtle.Turtle()
    t.pensize(3)
    t.speed(0)
    ...
    if depth == 2:
        koch_line(t.xcor(), t.ycor(), t.heading(),dir, size/3, depth + 1, limit)
        t.penup()
    t.forward(size/3)
    t.pendown()
    ...
    if depth < limit:
        ...
        koch_triangle(t.xcor(), t.ycor(), t.heading(),dir, size/3, depth + 1, limit)
        t.penup()
    t.forward(size/3)
    t.pendown()
...
def koch_triangle(startx,starty,angle,dir,size,depth,limit):
    #if we are too deep recursively
    if depth > limit:
        #end function
        return
    ...
    for i in range(sides):
        ...
        if dir == 'left':
            ...
            t.right(120)
        koch_line(t.xcor(), t.ycor(), t.heading(),op_dir, size/3, depth + 1, limit)
        t.forward(size/3)
        #draw smaller triangle coming off of side
        koch_triangle(t.xcor(), t.ycor(), t.heading(),op_dir, size/3, depth + 1, limit)
...
startx = 0
starty = 0 - height / 2
main_triangle(startx,starty,size)
triangle(startx,starty,size/2,2,6)
#koch_triangle(startx,starty,120,'right',size,1,4)
turtle.done()